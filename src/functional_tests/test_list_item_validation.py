...
class ItemValidationTest(FunctionalTest):
	...
	def test_cannot_add_duplicate_items(self):
		...
		def check_duplicate_item_error():
			assert self.get_error_element().text == \
	...
	def test_error_messages_are_cleared_on_input(self):
		...
		def check_error_displayed(is_displayed):
			assert is_displayed == self.get_error_element().is_displayed()
		self.wait_for(check_error_displayed, True)

		# she starts typing in the input box to clear the error
		self.get_item_input_box().send_keys('a')

		# and is pleased to see the error message disappears
		self.wait_for(check_error_displayed, False)